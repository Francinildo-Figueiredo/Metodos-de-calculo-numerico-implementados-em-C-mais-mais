...
int main(){
	...
	// Lendo as matrizes no padrão do Matlab
	...
	// Convertendo os valores das strings para float e passando para a matriz B
	...
	// Anexando a matriz identidade a matriz A para que possa ser realizado o cálculo da matriz inversa
	for(int i(0); i < n; i++){
		for(int j(0); j < n; j++){
			if(i != j)
				A[i].push_back(0);
	...
	// Matriz que irá receber os multiplicadores de cada linha
